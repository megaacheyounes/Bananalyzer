...
export const parseManifest = async (xmlContent: string): Promise<ParsedManifest> => {
  debug('xml2json length:', xmlContent.length);
  const result = await parser.parseStringPromise(removeSpecialChars(xmlContent));

  return result;
};

const removeSpecialChars = (content: string) => content.replace('\ufeff', '');
